...
function Menu(props) {
    ...
    return (
        <>
            {subMenu ? (
                <SubMenu props = {props}option={option} subMenu={subMenu} setSubMenu={setSubMenu}/> // Pass selected option to SubMenu
            ) : (
 ...
 function SubMenu({ props, option, subMenu,  setSubMenu}) {
    ...
    return (
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%' }}>
            ...
            <Button onClick={()=>{setSubMenu(!subMenu)}}>close</Button>
            <Paper sx={{ padding: 2, width: '300px', backgroundColor: '#333', color: 'white' }}>
